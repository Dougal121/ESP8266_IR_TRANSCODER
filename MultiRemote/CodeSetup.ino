void handleCodeSetup(){
  ...
  for (uint8_t j=0; j<server.args(); j++){
    ...
    i = String(server.argName(j)).indexOf("pad");
    if (i != -1){                                    // button number
      padselect = server.arg(j).toInt(); 
      padselect = constrain(padselect,0,MAX_VR);
    }
    ...
    if (i != -1){                                    // Display Name at top of page
     String(server.arg(j)).toCharArray( IRPad[padselect].pb[k].Display , sizeof(IRPad[padselect].pb[k].Display)) ;
    }

    i = String(server.argName(j)).indexOf("txcod");
    if (i != -1){                                    // transmitt code
     IRPad[padselect].pb[k].lCode = String(server.arg(j)).toInt() ;
    }
    
    i = String(server.argName(j)).indexOf("rxcod");
    if (i != -1){                                    // recieve code
     IRPad[padselect].pb[k].lRxCode = String(server.arg(j)).toInt() ;
    }
    ...
    if (i != -1){                                    // background color
     ...
     if ( ii != -1 ) {
       sscanf((char *)String(server.arg(j)).c_str(),"%X",&IRPad[padselect].pb[k].lBGColor);
     }else{
       IRPad[padselect].pb[k].lBGColor = String(server.arg(j)).toInt() ;
     }
    }
    ...
    if (i != -1){                                    // set last code to rx on button
     ...
     IRPad[padselect].pb[irset].wRxTxType &=  0x00ff  ;
     IRPad[padselect].pb[irset].wRxTxType |= (( ii & 0xff ) << 8 ) ;
     IRPad[padselect].pb[irset].lRxCode = (uint32_t)results.value ;
    }
    i = String(server.argName(j)).indexOf("txset");
    if (i != -1){                                    // set last code to tx on button
     ii = results.decode_type ; 
     IRPad[padselect].pb[irset].wRxTxType &=  0xff00  ;
     IRPad[padselect].pb[irset].wRxTxType |= ( ii & 0xff ) ;
     IRPad[padselect].pb[irset].lCode = (uint32_t)results.value ;
    }
...
//     Serial.print("SSID: ");     
    ...
    if (i != -1){                                    // set last code to tx on button
     ii = String(server.arg(j)).toInt() ; 
     IRPad[padselect].pb[irset].wRxTxType &= 0xff   ;
     IRPad[padselect].pb[irset].wRxTxType |= ((ii & 0xff ) << 8 ) ;
    }    
  ...
  server.sendContent(F("<h2><center><b>Virtual Remote - Code Setup</b></h2>"));

  server.sendContent("<table><tr><form method=get action=" + server.uri() + "><td>Selected Pad: </td><td><select name='pad'>");
  message = "" ;
  for (ii = 0; ii < MAX_VR ; ii++) {
    if ( padselect == ii ){
      MyColor = F(" SELECTED ");
    }else{
      MyColor = "";            
    }
    message += "<option value="+String(ii)+ MyColor +">" + IRPad[ii].Display ;          
  }
  server.sendContent(message);
  server.sendContent(F("</select></td><td><input type='submit' value='SET'></form></td></tr></table>"));
  ...
  for ( i = 0 ; i < MAX_PAD ; i++ ){
    ...
    SendFormTag();
    if ( IRPad[padselect].pb[i].lCode == 0 ){
      MyColor = "" ;
      MyColor2 = "" ;
    }else{
      if (IRPad[padselect].pb[i].lBGColor != 0 ) {
        sprintf(buff,"%06X",IRPad[padselect].pb[i].lBGColor);
      }else{
        sprintf(buff,"%06X",ghk.lDefButBGColor);      
      }
      MyColor = String("bgcolor=")+String(buff) ;
      if (IRPad[padselect].pb[i].lBGColor != 0 ) {                          // xor the color to get contrasting color
        sprintf(buff,"%06X",IRPad[padselect].pb[i].lBGColor ^ 0xffffff );
      }else{
    ...
    server.sendContent("<td><input type='text' name='descr' value='"+String(IRPad[padselect].pb[i].Display)+"' maxlength=7 size=7></td>");
    server.sendContent("<td "+MyColor+"><input type='text' name='bgcol' value='"+String(IRPad[padselect].pb[i].lBGColor)+"' size=6></td>");
    server.sendContent("<td><input type='text' name='rxcod' value='"+String(IRPad[padselect].pb[i].lRxCode)+"' size=6></td>");  
    server.sendContent("<td><input type='text' name='txcod' value='"+String(IRPad[padselect].pb[i].lCode)+"' size=6></td>") ;
    DecodeTypeLookup((long)((IRPad[padselect].pb[i].wRxTxType & 0xff00 )>> 8 ) ,(char *)&buff[0]);
    server.sendContent("<td><input type='text' name='wrxxx' value='"+String(((IRPad[padselect].pb[i].wRxTxType & 0xff00 )>> 8 ))+"' maxlength=2 size=2></td><td align=center>"+String(buff)+"</td>");  
    DecodeTypeLookup((long)(IRPad[padselect].pb[i].wRxTxType & 0xff ) ,(char *)&buff[0]);
    server.sendContent("<td><input type='text' name='wtxxx' value='"+String((IRPad[padselect].pb[i].wRxTxType & 0xff ))+"' maxlength=2 size=2></td><td align=center>"+String(buff)+"</td><td><input type='submit' value='SET'></td></form></tr>");  
  }
