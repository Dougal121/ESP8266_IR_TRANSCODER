...
//  SerialOutParams();
  ...
  for (uint8_t j=0; j<server.args(); j++){
    ...
    if (i != -1){  // 
      if ( lRebootCode == String(server.arg(j)).toInt() ){  // stop the phone browser being a dick and retry resetting !!!!
        ESP.restart() ;        
      }
    }
        
    i = String(server.argName(j)).indexOf("type");
    if (i != -1){  // 
...
//      Serial.println("IR Send RX " + String(webcode,HEX));
  ...
  if (bDefault) {         //  #####################################   Default Control ##############################################    
    ...
    server.sendContent("<table border=0 title='IR Tranmitter'><tr>"); 
    for ( int  j = 0 ; j < MAX_VR ; j++ ){
      if ( j == padselect ){
        if (IRPad[j].lBGHColor == 0 ) {
          sprintf(buff,"%06X",ghk.lDefButBGColor);          
        }else{
          sprintf(buff,"%06X",IRPad[j].lBGHColor);
        }
      }else{
        if (IRPad[j].lBGColor == 0 ) {
          sprintf(buff,"%06X",ghk.lDefButBGColor);          
        }else{
          sprintf(buff,"%06X",IRPad[j].lBGColor);
        }
      }
      server.sendContent( "<td align=center title='Pad "+String(j)+"'><form method=get action=" + server.uri() + "><input type='submit' value='"+String(IRPad[j].Display)+"' style='background-color:#"+String(buff)+"'><input type='hidden' name='pad' value='"+String(j)+"'></form></td>" ) ;                    
    }
    server.sendContent("</tr></table>"); 
    ...
    for ( k = 0 ; k < MAX_PAD ; k++ ){
      ...
      if ( IRPad[padselect].pb[k].lCode == 0 ){
        server.sendContent( "<td align=center title='"+String(k)+"'></td>" ) ;        
      }else{
//        server.sendContent( "<td align=center title='"+String(k)+"'><form method=get action=" + server.uri() + "><input type='submit' value='"+String(IRPad[padselect].pb[k].Display)+"' style='border-radius:8px; color:#"+String(ghk.lDefButTextColor,HEX)+";background-color:#"+String(ghk.lDefButBGColor,HEX)+"; height:"+String(ghk.butH)+"px; width:"+String(ghk.butW)+"px'><input type='hidden' name='code' value='"+String(IRPad[padselect].pb[k].lCode)+"'></form></td>" ) ;
        if (IRPad[padselect].pb[k].lBGColor != 0 ) {
          sprintf(buff,"%06X",IRPad[padselect].pb[k].lBGColor);
          server.sendContent( "<td align=center title='"+String(k)+"'><form method=get action=" + server.uri() + "><input type='submit' value='"+String(IRPad[padselect].pb[k].Display)+"' style='background-color:#"+String(buff)+"'><input type='hidden' name='code' value='"+String(IRPad[padselect].pb[k].lCode)+"'><input type='hidden' name='type' value='"+String(IRPad[padselect].pb[k].wRxTxType & 0xff)+"'></form></td>" ) ;          
        }else{
//          server.sendContent( "<td align=center title='"+String(k)+"'><form method=get action=" + server.uri() + "><input type='submit' value='"+String(IRPad[k].Display)+"' style='background-color:#"+String(ghk.lDefButBGColor,HEX)+"'><input type='hidden' name='code' value='"+String(IRPad[padselect].pb[k].lCode)+"'></form></td>" ) ;                    
          server.sendContent( "<td align=center title='"+String(k)+"'><form method=get action=" + server.uri() + "><input type='submit' value='"+String(IRPad[padselect].pb[k].Display)+"' ><input type='hidden' name='code' value='"+String(IRPad[padselect].pb[k].lCode)+"'><input type='hidden' name='type' value='"+String(IRPad[padselect].pb[k].wRxTxType & 0xff)+"'></form></td>" ) ;                    
        }
      }
