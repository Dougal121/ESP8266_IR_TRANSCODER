...
//  SerialOutParams();
  ...
  for (uint8_t j=0; j<server.args(); j++){
    ...
    if (i != -1){                                    // cheat - this always comes first
     iTmp = String(server.arg(j)).toInt() ;
    }
    i = String(server.argName(j)).indexOf("displ");
    if (i != -1){                                    // Display Name at top of page
     String(server.arg(j)).toCharArray( IRPad[iTmp].Display , sizeof(IRPad[iTmp].Display)) ;
    }
    i = String(server.argName(j)).indexOf("pbgcl");
    if (i != -1){                                    // background normal
     IRPad[iTmp].lBGColor = String(server.arg(j)).toInt() ;
    }
    i = String(server.argName(j)).indexOf("pbghl");
    if (i != -1){                                    // background highlight
     IRPad[iTmp].lBGHColor = String(server.arg(j)).toInt() ;
    }
  ...
  server.sendContent(F("<table border=1 title='General Setup'>"));
  ...
  server.sendContent(F("<tr><td>Default Background Color</td><td align=center>")) ; 
  SendFormTag();
  ...
  server.sendContent(F("Virtual Remote Pad Setup"));
  server.sendContent(F("<table border=1 title='Virtual Remote Pad Setup'>"));
  server.sendContent(F("<tr><th>Pad</th><th>Description</th><th>Normal<br>Color</th><th>Highlight<br>Color</th><th>.</th></tr>"));
  for (ii = 0 ; ii < MAX_VR ; ii++){
    if ( IRPad[ii].lBGColor == 0 ){
      sprintf(buff,"%06X",ghk.lDefButBGColor);            
    }else{
      sprintf(buff,"%06X",IRPad[ii].lBGColor);      
    }
    server.sendContent("<tr><td align=center bgcolor='"+String(buff)+"'>"+String(ii)+"</td><td><form method=get action=" + server.uri() + "><input type='hidden' name='pad' value='" + String(ii) + "'><input type='text' name='displ' value='"+String(IRPad[ii].Display)+"' maxlength=7></td>");
    server.sendContent("<td><input type='text' name='pbgcl' value='"+String(IRPad[ii].lBGColor)+"' size=8 maxlength=12></td>");
    server.sendContent("<td><input type='text' name='pbghl' value='"+String(IRPad[ii].lBGHColor)+"' size=8 maxlength=12></td><td><input type='submit' value='SET'></form></td></tr>");  
  }
  server.sendContent(F("</tr></table><br>"));
