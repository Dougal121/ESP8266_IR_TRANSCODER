...
} ghk_t ;                  //  general house keeping  27 bytes    (16x = 432 )
...
} IRPad_t ;                      //  general house keeping  22 bytes    (48x = 1056 )   16 x 48  768

IRPad_t  IRPad[MAX_PAD];
...
void setup() {
  ...
  EEPROM.begin(sizeof(ghk)+sizeof(IRPad)+64);  
...
//    }
  ...
  if (irrecv.decode(&results)) {
    ...
    for ( int  i = 0 ; i < MAX_PAD ; i++ ){
      if (( IRPad[i].lRxCode == (results.value & 0xFFFFFFFF))&&( IRPad[i].lRxCode != 0 )){        
        ircode = IRPad[i].lCode ;
        lTxNow = millis() + ghk.lTxDelay ;
/*        irsend.sendNEC(webcode, 32);
        digitalWrite(LEDR,LOW);  // flash red
        delay(1);
        digitalWrite(LEDR,HIGH);
        Serial.println("IR Send " + String(webcode,HEX));
        webcode = 0 ; */
