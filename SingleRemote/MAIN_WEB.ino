void SendHTTPHeader(bool bIncludeStyle){ 
  ...
  if ( bIncludeStyle ) {
    server.sendContent("<style>input[type=submit] { color:#"+String(ghk.lDefButTextColor,HEX)+";background-color:#"+String(ghk.lDefButBGColor,HEX)+"; height:"+String(ghk.butH)+"px; width:"+String(ghk.butW)+"px ; } </style>");
  }
...
//        server.sendContent( "<td align=center title='"+String(k)+"'><form method=get action=" + server.uri() + "><input type='submit' value='"+String(IRPad[k].Display)+"' style='height:"+String(ghk.butH)+"px; width:"+String(ghk.butW)+"px'><input type='hidden' name='code' value='"+String(IRPad[k].lCode)+"'></form></td>" ) ;
      }
